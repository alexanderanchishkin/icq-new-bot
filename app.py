...
def message_cb(bot, event):
    ...
    if(chat_id.find("@") < 0):
        bot.send_text(chat_id=chat_id, text="Бот создан для групповых бесед")
    else:
        if event.text=="/random":
            bot.send_text(chat_id=chat_id, text=str(randrange(101)))
        elif event.text == '/getjson':
            bot.send_text(chat_id=chat_id, text=str(event))
            ...
            kill_id = explorer.get_kill_id(user_id=chat_id)
            if(kill_id[0]):
            ...
            explorer.set_kill_id(user_id= chat_id, kill_id=json_response['msgId'])
